...
pkg는 nodejs가 설치되어 있지 않은 환경에서 실행할 수 있도록 standalone 의 바이너리 파일을 만들어서 배포해주는 모듈이다. 이번에 간단하게 엑셀파일을 비교하는 프로그램을 개발하고, 이를 비 개발자가 사용해야 될 일이 있어 진행 과정을 여기에 기록한다. 시스템에 NodeJS 설치 여부를 체크하여 아래와 같이 설치된 버전이 없다면 설치를 진행한다.
